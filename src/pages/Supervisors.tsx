...
import {
  UserCog, Users, Plus, Trash2, Pencil, Search, ShieldCheck, Phone, Loader2,
} from "lucide-react";
...
export default function Supervisors() {
  ...
  const getAgentsOf = (s: ApiSupervisor): Agent[] =>
    AGENTS.filter((a) => s.agentIds.includes(a.id));

  if (!isRealApi) {
    return (
      <AppLayout title="المشرفون">
          ...
          <CardHeader className="flex flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث..." className="pr-10 w-56" />
              </div>
                        ...
                        <TableRow key={s.id}>
                          <TableCell>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <Link to={`/supervisors/${s.id}`} className="text-sm font-bold hover:text-primary hover:underline">
                                  {s.name}
                                </Link>
                          ...
                          <TableCell>
                            <Badge variant="outline" className="bg-info/10 text-info border-info/30">{s.role}</Badge>
                          </TableCell>
                          <TableCell className="tabular-nums font-mono text-sm">
                            <span className="inline-flex items-center gap-1"><Phone className="w-3.5 h-3.5 text-muted-foreground" />{s.ext}</span>
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <div className="flex -space-x-2 -space-x-reverse">
                                {team.slice(0, 4).map((a) => (
                                  <Avatar key={a.id} className="w-7 h-7 ring-2 ring-background" title={a.name}>
                                    <AvatarFallback className="bg-muted text-[10px] font-bold">{a.avatar}</AvatarFallback>
                                  </Avatar>
                                ))}
                              </div>
                              <Badge variant="secondary" className="mr-2">{team.length} موظف</Badge>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center gap-1">
